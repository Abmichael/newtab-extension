...
class SettingsManager {
  ...
  async applySettings() {
    try {
      ...
      // Apply theme
      if (this.settings.theme && this.themes[this.settings.theme] && !this.settings.customTheme) {
  ...
  validateSetting(key, value) {
    const validations = {
      ...
      theme: (v) => typeof v === 'string' && (this.themes[v] || v === 'custom'),
      backgroundColor: (v) => typeof v === 'string' && /^#[0-9A-Fa-f]{6}$/.test(v),
      ...
      clockPosition: (v) => ['top-left', 'top-right', 'bottom-left', 'bottom-right'].includes(v),
  ...
  async importData(jsonString) {
    try {
      ...
      const newData = {
        ...
        settings: { ...this.getDefaultSettings(), ...(importData.settings || {}) },
