...
class UIManager {
  ...
  handleDrop(event) {
    ...
    if (this.draggedElement && folderId) {
      try {
        const afterElement = this.getDragAfterElement(event.clientY);
        const allFolders = Array.from(this.container.querySelectorAll('.folder-item:not(.dragging)'));
  ...
  getDragAfterElement(y) {
    const draggableElements = [...this.container.querySelectorAll('.folder-item:not(.dragging)')];
  ...
  showAddFolderDialog() {
    const dialog = this.createDialog('Add Folder', `
  ...
  showEditFolderDialog(folderId) {
    ...
    const dialog = this.createDialog('Edit Folder', `
      <div class="dialog-field">
        <label for="folder-name">Folder Name:</label>
      ...
      <div class="dialog-field">
        <label for="folder-color">Color:</label>
        <input type="color" id="folder-color" value="${folder.color || '#4285f4'}" />
      </div>
    `, [
      { text: 'Cancel', action: 'cancel' },
  ...
  showDeleteConfirmation(folderId) {
    ...
    const dialog = this.createDialog('Delete Folder', `
  ...
  showAddSiteDialog(folderId) {
    const dialog = this.createDialog('Add Site', `
      <div class="dialog-field">
  ...
  createDialog(title, content, buttons) {
    ...
    dialog.innerHTML = `
      ...
      <div class="dialog-buttons">
        ${buttons.map(btn => 
          `<button class="dialog-btn ${btn.primary ? 'primary' : ''} ${btn.destructive ? 'destructive' : ''}" 
                   data-action="${btn.action}" 
                   ${btn.action === 'cancel' ? 'aria-label="Cancel and close dialog"' : ''}>${btn.text}</button>`
        ).join('')}
      </div>
    `;
    ...
    dialog.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        dialog.dispatchEvent(new CustomEvent('action', { detail: { action: 'cancel' } }));
      }
    });
  ...
  createFolderElement(folder) {
    ...
    item.setAttribute('aria-label', `${folder.name} folder with ${folder.sites?.length || 0} sites`);
  ...
  initSettingsModal() {
    ...
    modal.addEventListener('click', (e) => {
      if (e.target.classList.contains('settings-tab')) {
        ...
        modal.querySelector(`[data-section="${tabName}"]`).classList.add('active');
      }
    });
  ...
  populateThemePreview(modal, settings) {
    ...
    themes.forEach(themeName => {
      ...
      themeEl.className = `theme-option ${settings.theme === themeName ? 'active' : ''}`;
      themeEl.dataset.theme = themeName;
      themeEl.style.background = theme.backgroundGradient;
      themeEl.style.color = theme.textColor;
      themeEl.textContent = themeName.charAt(0).toUpperCase() + themeName.slice(1);
      
      themeEl.addEventListener('click', () => {
        modal.querySelectorAll('.theme-option').forEach(t => t.classList.remove('active'));
  ...
  populateSettingsValues(modal, settings) {
    ...
    // Accessibility
    modal.querySelector('#high-contrast').checked = settings.accessibility?.highContrast || false;
    modal.querySelector('#reduced-motion').checked = settings.accessibility?.reducedMotion || false;
  }
  ...
  async saveSettings(modal) {
    try {
      ...
      if (selectedTheme && !modal.querySelector('#custom-theme').checked) {
        ...
        newSettings.backgroundColor = modal.querySelector('#background-color').value;
  ...
  async exportData() {
    try {
      ...
      a.href = url;
      a.download = `neotab-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  async importData(file) {
    ...
    try {
      ...
      this.closeSettingsModal();
      this.showNotification('Settings imported successfully! Refreshing...', 'success');
  ...
  async resetSettings(modal) {
    if (confirm('Are you sure you want to reset all settings to defaults? This cannot be undone.')) {
      try {
        ...
        this.showNotification('Error resetting settings. Please try again.', 'error');
      }
    }
  }
