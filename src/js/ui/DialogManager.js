...
class DialogManager extends ComponentManager {
				...
				try {
					...
					alert(
						err.message === "Invalid URL"
							...
							: "Failed to fetch page title. Link not added.",
					);
					setLoading(false);
	...
	showDeleteLinkDialog(linkId) {
		...
		// Check if this link was auto-added from top sites
		const isFromTopSites = link.metadata && link.metadata.fromTopSites === true;
      ...
      ${
				isFromTopSites
					? `
        <div style="margin-top: 12px; padding: 10px; background: rgba(99, 179, 237, 0.1); border-radius: 4px;">
          <label style="display: flex; align-items: center; cursor: pointer; user-select: none;">
            <input type="checkbox" id="blacklist-checkbox" style="margin-right: 8px; cursor: pointer;">
            <span>Don't suggest this site again from top sites</span>
          </label>
        </div>
      `
					: ""
    ...
    `;
		...
		const dialog = this.createDialog("Delete Link", dialogContent, [
			{ text: "Cancel", action: "cancel" },
			{ text: "Delete", action: "delete", primary: true, destructive: true },
		]);
		...
		dialog.addEventListener("action", async (e) => {
			if (e.detail.action === "delete") {
				try {
					// Check if user wants to blacklist
					const blacklistCheckbox = dialog.querySelector("#blacklist-checkbox");
					if (blacklistCheckbox?.checked) {
						await this.folderSystem.addToTopSitesBlacklist(link.url);
					}
