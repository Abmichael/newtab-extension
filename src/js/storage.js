...
class StorageManager {
		...
		try {
			...
			if (
				...
				(typeof data.meta !== "object" ||
					...
					(data.meta.topSitesBlacklist !== undefined &&
						!Array.isArray(data.meta.topSitesBlacklist)))
			) {
				return false;
			}
			...
			if (settings.clockFreePosition) {
				...
				if (
					typeof cfp !== "object" ||
					typeof cfp.xPercent !== "number" ||
					typeof cfp.yPercent !== "number"
				) {
					return false;
				}
				if (
			...
			if (settings.searchEngine) {
				if (
					typeof settings.searchEngine !== "object" ||
					typeof settings.searchEngine.name !== "string" ||
					typeof settings.searchEngine.template !== "string" ||
					!settings.searchEngine.template.includes("{query}")
				) {
					return false;
				}
			}
	...
	migrateData(data) {
		try {
			...
			if (!Array.isArray(migratedData.meta.topSitesBlacklist)) {
				migratedData.meta.topSitesBlacklist = [];
			}
