...
class ErrorHandler {
  constructor() {
    ...
    this.isDevelopment = chrome.runtime && chrome.runtime.getManifest().version.includes('dev');
    this.init();
  }
  ...
  handleError(errorInfo) {
    ...
    if (this.isDevelopment) {
      ...
      console.error('An error occurred. Error ID:', this.generateErrorId(error));
    }
  ...
  handleStorageError(operation, error) {
    ...
    if (operation === 'quota_exceeded') {
      this.showUserMessage('Storage is full. Please remove some folders or sites.', 'warning');
      return false;
    }
  ...
  async recoverStorage() {
    try {
      ...
      this.showUserMessage('Storage error. Please restart the browser.', 'error');
    }
  }
